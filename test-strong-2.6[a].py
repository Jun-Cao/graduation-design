...
dropout_rate = tf.placeholder(tf.float32,name='do_rate')
...
def inference(input_tensor,train,regularizer):
    ...
    with tf.variable_scope('layer3-conv2'):
        ...
        tf.summary.histogram("3-relu2", relu2)

    #第四层：池化层，过滤器的尺寸为2×2，使用全0补充，步长为2。
    #尺寸变化：10×10×6->5×5×16
    with tf.variable_scope('layer4-pool2'):
        pool2 = tf.nn.max_pool(relu2,ksize=[1,2,2,1],strides=[1,2,2,1],padding='SAME')
        tf.summary.histogram("4-pool2", pool2)
    ...
    with tf.variable_scope('layer5-fc1'):
        ...
        if train:
            ...
            tf.summary.histogram("5-fc1", fc1)    
    ...
    with tf.variable_scope('layer6-fc2'):
        ...
        if train:
            ...
            tf.summary.histogram("6-fc2", fc2)
    ...
    with tf.variable_scope('layer7-fc3'):
        ...
        fc3_biases = tf.get_variable('bias',[21],initializer=tf.truncated_normal_initializer(stddev=0.1))

        logit = tf.matmul(fc2,fc3_weights) + fc3_biases
        tf.summary.histogram("7-fc3", logit)
    return logit
...
#输出识别结果
result = tf.cast(tf.argmax(y,1),tf.int32, name="results")
...
with tf.Session() as sess:
    ...
    for i in range(train_num):
        ...
        for train_data_batch,train_label_batch in get_batch(train_data,train_label,batch_size):
            _,err,acc,mer = sess.run([train_op,loss,accuracy,merged],feed_dict={x:train_data_batch,y_:train_label_batch,dropout_rate:0.5})
